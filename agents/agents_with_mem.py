...
async def process_transaction_from_kafka(transaction: Transaction, compiled_workflow, checkpointer):
    ...
    try:
        ...
        try:
            ...
            if existing_state and existing_state.values:
                ...
                initial_state = WorkflowState(transaction=transaction)
...
def kafka_transaction_callback(transaction: Transaction, compiled_workflow, checkpointer):
    ...
    try:
        ...
        if loop.is_running():
            ...
            asyncio.create_task(process_transaction_from_kafka(transaction, compiled_workflow, checkpointer))
        else:
            # If no event loop is running, run the coroutine
            loop.run_until_complete(process_transaction_from_kafka(transaction, compiled_workflow, checkpointer))
    except Exception as e:
    ...
    async with AsyncMongoDBSaver.from_conn_string(
        ...
        try:
            ...
            try:
                ...
                # Create a callback function that includes the compiled workflow and checkpointer
                def kafka_callback(transaction: Transaction):
                    return kafka_transaction_callback(transaction, compiled_workflow, checkpointer)
                
                # Start consuming transactions from Kafka
                kafka_service.consume_transactions(kafka_callback)
